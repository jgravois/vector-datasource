...
def create_yaml_from_rules(kind_rules, min_zoom_rules):
    ...
    for kind_rule, min_zoom_rule in zip(kind_rules, min_zoom_rules):
        ...
        if rule.not_equals:
            ...
            for k, v in rule.not_equals:
                if table is None:
                    ...
                    assert table == k.table
                not_equals_filter[k.key] = '-%s' % v
        if rule.exists:
            # exists_filter = dict([(x, True) for x in rule.exists])
            exists_filter = {}
            for k in rule.exists:
                if table is None:
                    table = k.table
                else:
                    assert table == k.table
                exists_filter[k.key] = True
        if rule.not_exists:
            # not_exists_filter = dict([(x, False) for x in rule.not_exists])
            not_exists_filter = {}
            for k in rule.not_exists:
                if table is None:
                    table = k.table
                else:
                    assert table == k.table
                not_exists_filter[k.key] = False
        if rule.set_memberships:
            # any_of_filter = rule.set_memberships
            any_of_filter = {}
            for k, v in rule.set_memberships:
                if table is None:
                    table = k.table
                else:
                    assert table == k.table
                any_of_filter[k.key] = v
        if rule.default_rule:
        ...
        if not_equals_filter:
            yaml_filter.update(not_equals_filter)
            # not_datum = None
        ...
        try:
            ...
            min_zoom_val = min_zoom_rule.calc

        yaml_datum['min_zoom'] = min_zoom_val
        if table is not None:
            yaml_datum['table']= table
